...
public class FirstPersonAIO : MonoBehaviour {
    ...
    public void ControllerPause(){
        ...
        if(lockAndHideCursor){
            ...
            TimeManager.isPaused = controllerPauseState;
            pausePanel.SetActive(controllerPauseState);
        }
    }
    ...
    public class FPAIO_Editor : Editor{
        ...
        #region Pause Setup
        EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
        GUILayout.Label("Pause Setup", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));
        EditorGUILayout.Space();
        t.pausePanel = (GameObject)EditorGUILayout.ObjectField(new GUIContent("Pause Panel"), t.pausePanel, typeof(GameObject), true);
        GUI.enabled = t.pausePanel;
        #endregion
